...
export class Util {
    ...
    static Export(array: string[][], filePath: string) {
    ...
    static splitInput(arr: string[]): string[][] {
        const input = [];
    ...
    static inBounds(arr: any[], i: number, j: number | null = null): boolean {
        if (i < 0 || arr.length <= i) {
            return false;
        }
        if (j != null) {
    ...
    static create2DArray<T>(x: number, y: number, initialValue: T): T[][] {
        return new Array(x)
    ...
    static async numberImage(
        ...
        const img = await (Jimp as any).create({
